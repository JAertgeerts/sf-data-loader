...
class BulkApi extends EventEmitter {
  ...
  // Returns the CURRENT batch success. Generally this should be called
  // after finalState.
  // Returns true if we are in batch final state and ALL batches are in
  // 'Completed' state, AND no records failed.
  isBatchSuccess() {
    let rv = true;
    this.state.batchInfos.forEach((bi) => {
      if (bi.state !== 'Completed' || bi.numberRecordsFailed > 0) {
        rv = false;
      }
    });
    return rv;
